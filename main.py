...
# ================= KẾT NỐI DATABASE =================
conn = pyodbc.connect(
    r"DRIVER={SQL Server};SERVER=THAFIE\SQLEXPRESS;DATABASE=QuizDB;Trusted_Connection=yes;TrustServerCertificate=yes;"
)
...
def get_results(user_id):
    try:
        ...
        for row in cursor.fetchall():
            data.append({
                "exam": row.name if row.name else "N/A",
                "score": row.score,
                "total": row.total,
                "date": str(row.created_at)
            })
...
def get_user(user_id):
    try:
        ...
        cursor.execute("""
            SELECT id, username, fullname, student_code, role
            FROM Users
            WHERE id = ?
        """, (user_id,))
...
def update_user(user_id):
    try:
        ...
        if password:
            cursor.execute("""
                UPDATE Users
                SET fullname = ?, password = ?, student_code = ?
                WHERE id = ?
            """, (fullname, password, student_code, user_id))
        else:
            cursor.execute("""
                UPDATE Users
                SET fullname = ?, student_code = ?
                WHERE id = ?
            """, (fullname, student_code, user_id))
...
def dashboard(user_id):
    try:
        ...
        # 1. ĐÃ LÀM
        cursor.execute("""
            SELECT e.id, e.name, r.score, r.total, r.created_at
            FROM Results r
            JOIN Exams e ON r.exam_id = e.id
            WHERE r.user_id = ?
        """, (user_id,))
        ...
        cursor.execute("""
            ...
            FROM Exams
            WHERE id NOT IN (
                SELECT exam_id FROM Results WHERE user_id = ?
            )
        """, (user_id,))
...
#Giảng viên xem điểm tất cả sinh viên
    ...
    try:
        ...
        for row in cursor.fetchall():
            data.append({
                ...
                "fullname": row.fullname,
                "student_code": row.student_code,
                "exam": row.exam_name if row.exam_name else "-",
                "score": row.score,
                "total": row.total,
                "date": str(row.created_at)
            })
